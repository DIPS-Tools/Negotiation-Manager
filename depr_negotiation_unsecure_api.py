...
@app.post("/secure/negotiation/create", summary="Create a negotiation")
async def secure_create_upcast_negotiation(
        user_id: str = Header(..., description="The ID of the user"),
        body: UpcastPolicyObject = Body(..., description="The request object"),
        current_user: User = Depends(get_current_user)  # Protect the endpoint with JWT auth
):
    try:
...
async def get_upcast_offers(
    ...
    from bson import ObjectId  # You will need this to work with ObjectId in Python.

    offers = await policy_collection.aggregate([
        {
            '$match': {
                'type': 'offer',
                '$or': [
        ...
        },
        {
            '$addFields': {
                'producer_id_obj': {
                    '$toObjectId': '$producer_id'  # Converts producer_id string to ObjectId
                }
            }
        },
        {
            '$lookup': {
                ...
                'localField': 'producer_id_obj',  # Now using the ObjectId field
                'foreignField': '_id',  # Matching against _id in the users collection
                'as': 'producer_info'  # The field that will hold the user data
            }
        },
        {
            '$unwind': {
                'path': '$producer_info',  # Flattens the producer_info array
                'preserveNullAndEmptyArrays': True  # Keeps documents even if there's no producer match
            }
        },
        {
            '$set': {
                'producer_name': '$producer_info.name'  # Adds the producer's name to the policy document
            }
        }
    ]).to_list(length=None)
